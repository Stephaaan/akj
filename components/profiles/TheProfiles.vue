<template>
  <div class="font-worksans">
    <div v-bind:key="profile.slug" v-for="profile in profiles" class="flex to-reverse items-center px-4 sm:px-20 py-8">
      <img :src="profile.content.fotka" alt="profile_photo" class="h-24 md: h-56">
      <div class="items-center xs:ml-8 h-20 sm:h-48 skew">
        <div class="text-textPink font-bold text-base md:text-3xl">{{ profile.content.Meno }}</div>
