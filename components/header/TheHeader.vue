<template>
  <div :class="hamburgerOpen ? 'fixed' : 'relative'" class="w-full max-w-full flex justify-between z-50 pt-8">
    <div class="relative">
      <nuxt-link to="/">
      ...
      <div class="font-worksans text-sm uppercase text-textPink">
        ...
        <div id="hamburger_icon" class="px-8 md:hidden flex items-end flex-col z-10">
          ...
          <div
            ...
            class="flex w-screen h-screen bg-blackOpacity flex-col z-50 fixed right-0 top-100"
          >
            <div v-on:click.prevent="" class="w-full px-8 bg-white opacity-100 pb-8">
              <div v-for="link in links" v-bind:key="link.link" class="cursor-pointer">
                <nuxt-link :to="link.link">
                  {{ $t(link.text) }}
                </nuxt-link>
